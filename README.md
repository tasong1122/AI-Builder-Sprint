...
직접 회원가입하지 않고 아래 테스트용 계정으로 로그인할 수 있습니다. 두 계정을 각각 사용하면 돈 약속 공유, 상대방 동의, 독촉 메시지와 알림 흐름을 확인할 수 있습니다.

| 구분 | 이메일 | 비밀번호 |
| --- | --- | --- |
| 테스트 계정 1 | `test1@test.com` | `passwd1122` |
| 테스트 계정 2 | `test2@test.com` | `passwd1122` |
...
- 키 해시 등록 문의: **202555306@pusan.ac.kr**
...
개발 과정에서 AI를 다음과 같이 활용했습니다.

1. 먼저 직접 발견한 문제와 예상 해결 방법을 AI에 설명하고, 해당 아이디어가 실제로 구현 가능한지 판단하도록 했습니다.
2. 구현 가능성을 확인한 뒤 AI에게 대략적인 이용자 흐름과 기능 파이프라인을 작성하게 했습니다. 제안된 내용은 직접 검토하고 서비스 방향에 맞게 수정했습니다.
3. 확정한 흐름과 선호하는 UI 스타일을 바탕으로, AI 에이전트가 따라야 할 개발 지침서인 [`AGENTS.md`](AGENTS.md)를 작성하도록 했습니다.
4. 필요한 기능의 데이터 구조를 분석한 다음, Firebase 필드와 각 데이터가 의미하는 내용을 [`DATA_STRUCTURE.md`](DATA_STRUCTURE.md)로 정리하도록 했습니다.
5. 마지막으로 AI 에이전트에게 두 문서를 먼저 읽게 하고, 정리된 기준에 맞춰 실제 코드를 구현하도록 요청했습니다. 구현 결과는 직접 확인하면서 필요한 부분을 다시 수정했습니다.

AI는 아이디어의 실현 가능성 검토, 사용자 흐름 정리, 개발 지침과 데이터 구조 문서화, 코드 구현을 보조했습니다. 문제 정의와 기획의 최종 결정, UI 방향 선택, 생성 결과의 검토와 수정은 개발자가 직접 담당했습니다.
...
결과: 설정 화면, 비밀번호 재설정 메일, 다크 모드, 둥근 상단 하단 바, Firebase 인증 세션 기반 자동 로그인이 구현되었습니다. 이메일 변경 기능은 최종 계획에서 제외했습니다.
